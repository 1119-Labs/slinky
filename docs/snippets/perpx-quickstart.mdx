<AccordionGroup>
    <Accordion title="Perpx v1">

        **The minimum required version for Connect with Perpx v1 is `v1.0.5+`.**
        ...
        We will also supply an edited configuration for the `perpx_migration_api` which facilitates graceful migration from perpx's `x/prices` to `x/marketmap`.
        ...
        **Make sure to fill in the URL for the REST endpoint and gRPC endpoint of your node** (in that order). The migration API will _not_ work if the REST API endpoint is not the first endpoint in the list.
        ...
        ```shell
        slinky \
        --marketmap-provider perpx_migration_api \
        --oracle-config path/to/oracle.json
        ```
    </Accordion>
    <Accordion title="Perpx (Legacy)">

        **The minimum required version for Connect with Perpx (Legacy) is `v1.0.5+`.**
        ...
        ```
        With your Perpx node's REST API endpoint and the `oracle.json` file path, enter the following command to run Connect.

        ```shell
        slinky \
            --marketmap-provider perpx_api \
            --market-map-endpoint https://<perpx-node REST API endpoint> \
            --oracle-config path/to/oracle.json
        ```
    </Accordion>
</AccordionGroup>
