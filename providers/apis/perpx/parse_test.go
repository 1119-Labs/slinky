...
import (
	...
	coinbasews "github.com/1119-Labs/slinky/providers/websockets/coinbase"
...
func TestConvertMarketParamsToMarketMap(t *testing.T) {
	testCases := []struct {
		name     string
		params   perpxtypes.QueryAllMarketParamsResponse
		...
		},
		{
			...
			params: perpxtypes.QueryAllMarketParamsResponse{
				MarketParams: []perpxtypes.MarketParam{
					{
						Pair:               "BTC-USD", // Taken from Perpx mainnet
						Exponent:           -5,
						MinExchanges:       3,
					...
					{
						Pair:               "ETH-USD", // Taken from Perpx mainnet
						MinExchanges:       3,
						Exponent:           -6,
					...
					{
						Pair:               "USDT-USD", // Taken from Perpx mainnet
						MinExchanges:       3,
						Exponent:           -9,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			resp, err := perpx.ConvertMarketParamsToMarketMap(tc.params)
			if tc.err {
				require.Error(t, err)
			} else {
...
func TestCreateCurrencyPairFromMarket(t *testing.T) {
	t.Run("good ticker", func(t *testing.T) {
		pair := "BTC-USD"
		cp, err := perpx.CreateCurrencyPairFromPair(pair)
		require.NoError(t, err)
	...
	t.Run("bad ticker", func(t *testing.T) {
		pair := "BTCUSD"
		_, err := perpx.CreateCurrencyPairFromPair(pair)
		require.Error(t, err)
	})
...
func TestCreateTickerFromMarket(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			ticker, err := perpx.CreateTickerFromMarket(tc.market)
			if tc.err {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
...
func TestConvertExchangeConfigJSON(t *testing.T) {
	testCases := []struct {
		name              string
		config            perpxtypes.ExchangeConfigJson
	...
	}{
		{
			...
			config: perpxtypes.ExchangeConfigJson{
				Exchanges: []perpxtypes.ExchangeMarketConfigJson{
					{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			providers, err := perpx.ConvertExchangeConfigJSON(tc.config)
			if tc.expectedErr {
				require.Error(t, err)
				return
			}
...
func TestExtractMetadata(t *testing.T) {
	testcases := []struct {
		name             string
		providerName     string
		cfg              perpxtypes.ExchangeMarketConfigJson
	...
	}{
		{
			...
			cfg:              perpxtypes.ExchangeMarketConfigJson{Ticker: "BTC-USDT"},
			expectedMetadata: "",
			expectedErr:      false,
		},
		{
		...
		},
		{
			...
			providerName:     raydium.Name,
			cfg:              perpxtypes.ExchangeMarketConfigJson{Ticker: "SMOLE-SOL-abc-6-def-7"},
			expectedMetadata: "",
			expectedErr:      true,
		},
		{
			...
			providerName:     raydium.Name,
			cfg:              perpxtypes.ExchangeMarketConfigJson{Ticker: "SMOLE-SOL-VDZ9kwvKRbqhNdsoRZyLVzAAQMbGY9akHbtM6YugViS-6-def-7"},
			expectedMetadata: "",
			expectedErr:      true,
		},
		{
			...
			providerName: raydium.Name,
			cfg:          perpxtypes.ExchangeMarketConfigJson{Ticker: "SMOLE-SOL-VDZ9kwvKRbqhNdsoRZyLVzAAQMbGY9akHbtM6YugViS-a-HiLcngHP5y1Jno53tuuNeFHKWhyyZp3XuxtKPszD6rG2-7"},
			expectedErr:  true,
		},
		{
			name:         "raydium provider w/ incorrect base decimals",
			providerName: raydium.Name,
			cfg:          perpxtypes.ExchangeMarketConfigJson{Ticker: "SMOLE-SOL-VDZ9kwvKRbqhNdsoRZyLVzAAQMbGY9akHbtM6YugViS-8-HiLcngHP5y1Jno53tuuNeFHKWhyyZp3XuxtKPszD6rG2-a"},
			expectedErr:  true,
		},
		{
			name:         "raydium provider w/ incorrect open-orders account",
			providerName: raydium.Name,
			cfg:          perpxtypes.ExchangeMarketConfigJson{Ticker: "SMOLE-SOL-VDZ9kwvKRbqhNdsoRZyLVzAAQMbGY9akHbtM6YugViS-8-HiLcngHP5y1Jno53tuuNeFHKWhyyZp3XuxtKPszD6rG2-9-a-5EgCcjkuE42YyTZY4QG8qTioUwNh6agTvJuNRyEqcqV1"},
			expectedErr:  true,
		},
		{
			name:         "raydium provider w/ incorrect ammId account",
			providerName: raydium.Name,
			cfg:          perpxtypes.ExchangeMarketConfigJson{Ticker: "SMOLE-SOL-VDZ9kwvKRbqhNdsoRZyLVzAAQMbGY9akHbtM6YugViS-8-HiLcngHP5y1Jno53tuuNeFHKWhyyZp3XuxtKPszD6rG2-9-FeKBjZ5rBvHPyppHf11qjYxwaQuiympppCTQ5pC6om3F-a"},
			expectedErr:  true,
		},
		{
			name:             "raydium provider w/ correct metadata",
			providerName:     raydium.Name,
			cfg:              perpxtypes.ExchangeMarketConfigJson{Ticker: "SMOLE-SOL-VDZ9kwvKRbqhNdsoRZyLVzAAQMbGY9akHbtM6YugViS-8-HiLcngHP5y1Jno53tuuNeFHKWhyyZp3XuxtKPszD6rG2-9-FeKBjZ5rBvHPyppHf11qjYxwaQuiympppCTQ5pC6om3F-5EgCcjkuE42YyTZY4QG8qTioUwNh6agTvJuNRyEqcqV1"},
		...
		},
		{
			...
			cfg:          perpxtypes.ExchangeMarketConfigJson{Ticker: "0xabc123-abc"},
			expectedErr:  true,
		},
		{
			name:         "uniswapv3-ethereum invalid base decimals",
			providerName: uniswapv3.ProviderNames[constants.ETHEREUM],
			cfg:          perpxtypes.ExchangeMarketConfigJson{Ticker: "0xabc123-abc-12"},
			expectedErr:  true,
		},
		{
			name:         "uniswapv3-ethereum invalid quote decimals",
			providerName: uniswapv3.ProviderNames[constants.ETHEREUM],
			cfg:          perpxtypes.ExchangeMarketConfigJson{Ticker: "0xabc123-8-abc"},
			expectedErr:  true,
		},
		{
			name:         "uniswapv3-ethereum invalid pool address",
			providerName: uniswapv3.ProviderNames[constants.ETHEREUM],
			cfg:          perpxtypes.ExchangeMarketConfigJson{Ticker: "zzzzzz-8-18"},
			expectedErr:  true,
		},
		{
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			metadata, err := perpx.ExtractMetadata(tc.providerName, tc.cfg)
			if tc.expectedErr {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
