...
// APIHandler implements the MarketMapAPIDataHandler interface for the Perpx prices module, which can be used
// by a base provider. This is specifically for fetching market data from the Perpx prices module, which is
// then translated to a market map.
type APIHandler struct {
	logger *zap.Logger

	// api is the api config for the Perpx market params API.
...
// a single market map object that was converted from the Perpx market params response.
func (h *APIHandler) ParseResponse(
	...
	if resp == nil {
		h.logger.Debug("got nil response from perpx market params API")
		return types.NewMarketMapResponseWithErr(
			chains,
			providertypes.NewErrorWithCode(
	...
	// Parse the response body into a perpx market params response object.
	var params perpxtypes.QueryAllMarketParamsResponse
	if err := json.NewDecoder(resp.Body).Decode(&params); err != nil {
		h.logger.Debug("failed to parse perpx market params response", zap.Error(err))
		return types.NewMarketMapResponseWithErr(
			chains,
			providertypes.NewErrorWithCode(
				fmt.Errorf("failed to parse perpx market params response: %w", err),
				providertypes.ErrorFailedToDecode,
			),
		)
	}

	// Convert the perpx market params to a market map.
	marketResp, err := ConvertMarketParamsToMarketMap(params)
	if err != nil {
		h.logger.Debug(
			"failed to convert perpx market params to market map",
		...
		return types.NewMarketMapResponseWithErr(
			chains,
			providertypes.NewErrorWithCode(
				fmt.Errorf("failed to convert perpx market params to market map: %w", err),
