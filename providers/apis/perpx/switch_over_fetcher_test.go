...
import (
	...
	apimetricsmocks "github.com/1119-Labs/slinky/providers/base/api/metrics/mocks"
...
func TestDefaultSwitchOverProvider(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			_, err := perpx.NewDefaultSwitchOverMarketMapFetcher(tc.logger, tc.api, tc.rh, tc.metrics)
			if tc.err {
				require.Error(t, err)
				return
			}
...
func TestNewSwitchOverAPIHandler(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			_, err := perpx.NewSwitchOverFetcher(tc.logger, tc.pricesFetcher, tc.marketmapFetcher, metrics.NewNopAPIMetrics())
			if tc.err {
				require.Error(t, err)
				return
			}
...
func TestSwitchOverProvider_Fetch(t *testing.T) {
	...
	fetcher, err := perpx.NewSwitchOverFetcher(zap.NewNop(), pf, mmf, metrics)
	require.NoError(t, err)
	...
	cases := []struct {
	...
	}{
		{
			...
			pricesFetcher: func(pf *apihandlermocks.APIFetcher[mmclient.Chain, *mmtypes.MarketMapResponse]) {
				resp := mmclient.NewMarketMapResponse(
					mmclient.ResolvedMarketMap{
						perpx.PERPXChain: mmclient.NewMarketMapResult(
			...
			metrics: func(m *apimetricsmocks.APIMetrics) {
				m.On("AddProviderResponse", perpx.Name, strings.ToLower(perpx.PERPXChain.String()), providertypes.OK).Once()
			},
			resp: mmclient.NewMarketMapResponse(
				mmclient.ResolvedMarketMap{
					perpx.PERPXChain: mmclient.NewMarketMapResult(
		...
		},
		{
			...
			metrics: func(m *apimetricsmocks.APIMetrics) {
				m.On("AddProviderResponse", marketmap.Name, strings.ToLower(perpx.PERPXChain.String()), providertypes.OK).Once()
			},
			resp: mmclient.NewMarketMapResponse(
				mmclient.ResolvedMarketMap{
					perpx.PERPXChain: mmclient.NewMarketMapResult(
		...
		},
		{
			...
			marketmapFetcher: func(mmf *apihandlermocks.APIFetcher[mmclient.Chain, *mmtypes.MarketMapResponse]) {
				resp := mmclient.NewMarketMapResponse(
					...
					mmclient.UnResolvedMarketMap{
						perpx.PERPXChain: providertypes.UnresolvedResult{
			...
			metrics: func(m *apimetricsmocks.APIMetrics) {
				m.On("AddProviderResponse", marketmap.Name, strings.ToLower(perpx.PERPXChain.String()), providertypes.ErrorAPIGeneral).Once()
			},
			resp: mmclient.NewMarketMapResponse(
				make(mmclient.ResolvedMarketMap),
				mmclient.UnResolvedMarketMap{
